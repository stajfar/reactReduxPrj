...
import { updatePost, Post, selectPostById } from './PostsSlice';
...
function EditPost() {
    ...
    //make it one liner
    const [formLoadeState, setFormLoadState] = useState(useSelector((state: any) => selectPostById(state, id ?? '')) ?? {}); 
    ...
    useEffect(() => {
        ...
        setSubmissionState(formLoadeState);
    ...
    const handleFormSubmit = (data: IChangeEvent, event: FormEvent) => {
        if (data.formData) {
            ...
            const myNewPost: Post = { userId: entry.user, id: entry.id, title: entry.title, body: entry.body };


            const submitPosts = async () => {
                try {
                    //async the global state
                    dispatch(updatePost(myNewPost));
